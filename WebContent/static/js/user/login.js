$(function() {
    ...
    $("#submit").hover(
        function() {
            $("#submit").attr("src", "/gcoads/static/images/login2.jpg");
        },
        function() {
            $("#submit").attr("src", "/gcoads/static/images/login1.jpg");
        }
    );
...
function validateLoginname() {
    ...
    if(!value) {// 非空校验
        ...
        bool = false;
    } else if(value.length < 3 || value.length > 18) {//长度校验
        $("#loginnameError").css("display", "");
        $("#loginnameError").text("用户名长度必须在3 ~ 18之间！");
        bool = false;
    }
    return bool;
}
